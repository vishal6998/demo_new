...
class HeaderMinimal extends HeaderType {
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['menu_area_in_grid'] = laurent_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes';
		$parameters['fullscreen_menu_icon_class'] = laurent_elated_get_fullscreen_menu_icon_class();
		
		$parameters = apply_filters( 'laurent_elated_filter_header_minimal_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$menu_background_color             = laurent_elated_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = laurent_elated_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = laurent_elated_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = laurent_elated_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $menuAreaTransparent ) {
			...
			if ( ( $sliderExists && laurent_elated_is_top_bar_enabled() )
			     || laurent_elated_is_top_bar_enabled() && laurent_elated_is_top_bar_transparent()
			) {
				$transparencyHeight += laurent_elated_get_top_bar_height();
			}
		}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$menu_background_color             = laurent_elated_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = laurent_elated_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = laurent_elated_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = laurent_elated_options()->getOptionValue( 'menu_area_grid_background_transparency' );
	...
	public function calculateHeaderHeight() {
		...
		if ( laurent_elated_is_top_bar_enabled() ) {
			$headerHeight += laurent_elated_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = laurent_elated_get_meta_field_intersect( 'header_behaviour' );
