...
/*
   Class: LaurentElatedClassMultipleImages
   A class that initializes Elated Multiple Images
*/

class LaurentElatedClassMultipleImages implements iLaurentElatedInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$laurent_elated_global_Framework->eltdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="eltdf-gallery-uploader">
								<a class="eltdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'laurent' ); ?></a>
								<a class="eltdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'laurent' ); ?></a>
							</div>
...
class LaurentElatedClassTwitterFramework implements iLaurentElatedInterfaceRender {
	public function render( $factory ) {
		$twitterApi = LaurentTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'laurent' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'laurent' ) : esc_html__( 'Connect with Twitter', 'laurent' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'laurent' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'laurent' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class LaurentElatedClassInstagramFramework implements iLaurentElatedInterfaceRender {
	public function render( $factory ) {
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'laurent' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'laurent' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'laurent' );
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'laurent' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'laurent' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ($instagram_user_id) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'laurent' ); echo esc_attr($instagram_user_id) ?></p>
							</div>
						</div>
					<?php } ?>
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'laurent' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'laurent' ); ?></a>
							</div>
							<!-- <div class="col-lg-4">
								<a class="btn btn-primary" href="<//?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><//?php echo esc_html__( 'Connect with Instagram Business account', 'laurent' ); ?></a>
							</div> -->
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'laurent' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class LaurentElatedClassRepeater implements iLaurentElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="eltdf-repeater-table-heading">
					<div class="eltdf-repeater-fields-holder">
						<div class="eltdf-repeater-table-cell eltdf-repeater-sort"><?php esc_html_e( 'Order', 'laurent' ) ?></div>
						...
						<div class="eltdf-repeater-table-cell eltdf-repeater-remove"><?php esc_html_e( 'Remove', 'laurent' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="eltdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="eltdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = laurent_elated_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class LaurentElatedClassFieldAddress extends LaurentElatedClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Laurent Options', 'laurent' ); ?></p>
							<?php } ?>
...
class LaurentElatedClassFieldIcon extends LaurentElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = laurent_elated_icon_collections()->getIconCollectionsEmpty();
