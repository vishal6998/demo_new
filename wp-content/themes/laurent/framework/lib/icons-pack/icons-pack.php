...
include LAURENT_ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( LAURENT_ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( laurent_elated_is_customizer_item_enabled( $module_load, 'laurent_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'laurent_elated_get_icon_pack_collections' ) ) {
	function laurent_elated_get_icon_pack_collections() {
		$options = apply_filters( 'laurent_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'laurent_elated_get_icon_pack_options' ) ) {
	function laurent_elated_get_icon_pack_options() {
		$options = apply_filters( 'laurent_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'laurent_elated_get_icon_pack_social_options' ) ) {
	function laurent_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'laurent_elated_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: LaurentElatedClassIconCollections
 */
class LaurentElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = laurent_elated_get_icon_pack_collections();
		$this->iconPackOptions       = laurent_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = laurent_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = laurent_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'laurent' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'laurent' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'laurent' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'laurent' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'laurent' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'laurent' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'laurent' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'laurent' )       => 'fa-lg',
			esc_html__( 'Small', 'laurent' )      => 'fa-2x',
			esc_html__( 'Medium', 'laurent' )     => 'fa-3x',
			esc_html__( 'Large', 'laurent' )      => 'fa-4x',
			esc_html__( 'Very Large', 'laurent' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iLaurentElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'laurent' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'laurent' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'laurent-elated-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
    ...
    public function getElementorParamsArray($control_object, $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false){
        ...
        $control_object->add_control(
            'icon_pack', [
                'label' => esc_html__( 'Icon Pack', 'laurent' ),
