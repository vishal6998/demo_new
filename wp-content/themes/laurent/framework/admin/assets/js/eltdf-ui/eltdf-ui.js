(function($){
	...
	function eltdfInitMediaUploader() {
		...
		function eltdfInitMediaRemoveBtn(btn) {
			btn.on('click', function () {
				...
				//reset meta fields
				btn.siblings('.eltdf-media-meta-fields').find('input[type="hidden"]').each(function () {
					$(this).val('');
				});
		...
		wp.media.customlibEditGallery1 = {
			frame: function () {
				...
				this._frame.on('update', function () {
					...
					var data = {
						action: 'laurent_elated_gallery_upload_get_images',
						ids: ids,
						post_name: $input_gallery_items.attr('name'),
						gallery_upload_get_images: $('#eltdf_gallery_upload_get_images_' + $input_gallery_items.attr('name')).val()
					};
	...
	function eltdfInitAjaxForm() {
		$('#eltdf_top_save_button').on('click', function () {
			$('.eltdf_ajax_form').trigger('submit');
		...
		$(document).on("submit", ".eltdf_ajax_form", function (a) {
			var b = $(this),
				c = {
					action: "laurent_elated_save_options"
				};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Options now?')) {
					...
					var data = {
						action: 'laurent_core_import_theme_options',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Custom Sidebars now?')) {
					...
					var data = {
						action: 'laurent_core_import_custom_sidebars',
						content: importValue,
						nonce: importNonce
					};
		...
		if (holder.length) {
			...
			eltdfImportBtn.on('click', function (e) {
				...
				if (confirm('Are you sure, you want to import Widgets now?')) {
					...
					var data = {
						action: 'laurent_core_import_widgets',
						content: importValue,
						nonce: importNonce
					};
