		...
		$('body').on('click', button_class, function () {
			...
			wp.media.editor.send.attachment = function (props, attachment) {
				if (_custom_media) {
					...
					parent.find('.eltdf-user-custom-media-url').val(attachment.id).trigger('change');
		...
		$('body').on('click', button_class, function () {
			...
			if (image.val() !== '' && confirm('Are you sure you want to remove this file?')) {
				var result = $.ajax({
					...
					type: 'GET',
					data: {
						action: 'laurent_elated_user_del_image',
						user_id: $this.data('userid'),
						field_name: image.attr('name'),
						user_del_image_nonce: $('input[name="eltdf_user_del_image_nonce"]').val()
					},
					dataType: 'text'
				});
