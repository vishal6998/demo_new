...
class ElatedSkin extends LaurentElatedClassSkinAbstract {
	
    /**
     * Skin constructor. Hooks to laurent_elated_admin_scripts_init and laurent_elated_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'elated';
		
		// Admin register scripts and styles
		add_action( 'laurent_elated_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'laurent_elated_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'laurent_elated_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'laurent_elated_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'laurent_elated_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'laurent_elated_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'laurent_elated_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			laurent_elated_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        laurent_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = laurent_elated_get_admin_tab();
        $active_page = $laurent_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see LaurentElatedClassSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param LaurentElatedClassAdminPage $page current page to load
	 * @see LaurentElatedClassSkinAbstract::loadTemplatePart()
	 */
