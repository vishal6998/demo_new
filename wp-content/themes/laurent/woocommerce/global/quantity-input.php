...
if ( $max_value && $min_value === $max_value ) {
	...
	$label = ! empty( $args['product_name'] ) ? sprintf( esc_html__( '%s quantity', 'laurent' ), wp_strip_all_tags( $args['product_name'] ) ) : esc_html__( 'Quantity', 'laurent' );
	?>
	<div class="eltdf-quantity-buttons quantity">
		...
		<input  type="text"
				...
				class="eltdf-quantity-input <?php echo esc_attr( join( ' ', (array) $classes ) ); ?>"
				data-step="<?php echo esc_attr( $step ); ?>"
				data-min="<?php echo esc_attr( $min_value ); ?>"
				data-max="<?php echo esc_attr( 0 < $max_value ? $max_value : '' ); ?>"
				name="<?php echo esc_attr( $input_name ); ?>"
				value="<?php echo esc_attr( $input_value ); ?>"
				aria-label="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'laurent' ); ?>"
