...
$row_content_width = $anchor = $content_text_aligment = $simple_background_color = $simple_background_image = $background_image_position = $disable_background_image = $parallax_background_image = $parallax_bg_speed = $parallax_bg_height = $pattern_svg_path = $pattern_svg_position = $pattern_svg_hr_offset = $pattern_svg_vr_offset = '';
...
if ( $row_content_width !== 'grid' ) {
	...
	if ( ! empty( $simple_background_image ) ) {
		...
		$eltdf_vc_row_style[] = 'background-image: url(' . esc_url( $image ) . ')';
	}
	...
	if ( ! empty( $simple_background_image ) ) {
		$image_src              = wp_get_attachment_image_src( $simple_background_image, 'full' );
		$image = isset($image_src[0]) ? $image_src[0] : '';
		$eltdf_grid_row_style[] = 'background-image: url(' . esc_url( $image ) . ')';
	}
...
if ( $row_content_width === 'grid' ) {
	$eltdf_row_wrapper_start .= '<div class="eltdf-row-grid-section-wrapper ' . esc_attr( $grid_row_classes ) . '" ' . esc_attr( $grid_row_datas ) . ' ' . laurent_elated_get_inline_style( $eltdf_grid_row_styles ) . '><div class="eltdf-row-grid-section">';
	$eltdf_row_wrapper_end   .= '</div></div>';
}

$eltdf_row_svg_pattern      = apply_filters('laurent_elated_filter_vc_row_svg_pattern', '', $atts);
$eltdf_after_wrapper_open   = apply_filters('laurent_elated_filter_vc_after_wrapper_open', '', $atts);
$eltdf_before_wrapper_close = apply_filters('laurent_elated_filter_vc_before_wrapper_close', '', $atts);
$css_classes                = apply_filters('laurent_elated_filter_vc_css_classes', $css_classes, $atts);
...
$output .= $eltdf_row_svg_pattern;
$output .= $eltdf_row_wrapper_start;
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . laurent_elated_get_inline_style( $eltdf_vc_row_styles ) . '>';
...
echo laurent_elated_get_module_part( $output );
