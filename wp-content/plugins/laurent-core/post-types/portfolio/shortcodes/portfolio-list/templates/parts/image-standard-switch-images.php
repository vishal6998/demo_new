...
<div class="eltdf-pli-image">
    ...
    <?php if (!empty($switch_featured_image)) {
        if ($thumb_size === 'full') { ?>
            <img itemprop="image" class="eltdf-pl-hover-image" src="<?php echo esc_url($switch_featured_image); ?>" alt="<?php esc_attr_e('Portfolio Hover Featured Image', 'laurent-core'); ?>" />
        <?php } else {
            $thumb_image_size = laurent_elated_get_image_size($thumb_size);
            echo laurent_elated_generate_thumbnail(null, $switch_featured_image, $thumb_image_size['width'], $thumb_image_size['height']);
        }
    } ?>
</div>