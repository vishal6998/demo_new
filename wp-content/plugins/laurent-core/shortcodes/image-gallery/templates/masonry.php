		...
		<?php foreach ( $images as $image ) { ?>
			...
			if ( ! empty( $image_size_class ) ) {
				...
				if ( $image_size_class === 'large-width-height' ) {
					...
					$new_image_size = 'laurent_elated_image_square';
				} else if ( $image_size_class === 'large-width' ) {
					$new_image_size = 'laurent_elated_image_landscape';
				} else if ( $image_size_class === 'large-height' ) {
					$new_image_size = 'laurent_elated_image_portrait';
				} else {
					$new_image_size = 'full';
				}
			}
			...
			<div class="eltdf-ig-image eltdf-item-space <?php echo esc_attr( $image_classes ); ?>">
				<div class="eltdf-ig-image-inner">
						<?php if ( is_array( $image_size ) && count( $image_size ) ) :
							echo laurent_elated_generate_thumbnail( $image['image_id'], null, $image_size[0], $image_size[1] );
						else:
