...
class LaurentElatedClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_laurent_core_action_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_laurent_core_action_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'laurent-core' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'laurent-core' );
		}
...
if ( ! function_exists( 'laurent_core_create_like' ) ) {
	function laurent_core_create_like() {
		LaurentElatedClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'laurent_core_create_like' );
}