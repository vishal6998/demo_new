(function ( $ ) {
	...
	var qodefDeactivationModal = {
		init: function () {
			this.deactivationLink  = $( '#the-list' ).find( '[data-slug="qode-quick-view-for-woocommerce"] span.deactivate a' );
			this.deactivationModal = $( '#qode-quick-view-for-woocommerce-deactivation-modal' );
		...
		initSubmitFunctionality: function () {
			var submitButton = this.deactivationModal.find( '.qodef-deactivation-modal-button-submit' ),
				...
				nonceHolder  = this.deactivationModal.find( '#qode-quick-view-for-woocommerce-deactivation-nonce' );
						...
						$.ajax(
							{
								type: 'POST',
								data: {
									action: 'qode_quick_view_for_woocommerce_deactivation',
									reason: reason,
